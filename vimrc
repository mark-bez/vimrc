...
set ignorecase                 " when used with smartcase makes searches not case sensitive except when using an uppercase character
...
set path=$PWD/**               " sets the default path used for :find and gf and others to the directory the current buffer is in only
set regexpengine=1
...
set shortmess=a                " turns off Bram's message on start-up and prevents Press Enter or type command to continue message on save when backupdir is being used
set showcmd
...
" add undo points at every punctuation mark when writing prose
inoremap ! !<C-g>u
inoremap , ,<C-g>u
inoremap . .<C-g>u
inoremap : :<C-g>u
...
" let g:indentLine_first_char = '' let g:indentLine_showFirstIndentLevel = 1
...
" It maps cd to change the working directory to the directory of active file, cs means current session – all windows - and cw means current window only
nnoremap <leader>cs :cd %:p:h<CR>
nnoremap <leader>cw :lcd %:h<CR>
